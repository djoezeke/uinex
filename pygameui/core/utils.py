...
def assert_color(
    color: Union[ColorInputType, List[int]], warn_if_invalid: bool = True
) -> ColorType:
    """
...
def assert_list_vector(
    list_vector: Union[List[Vector2NumberType], Tuple[Vector2NumberType, ...]],
    length: int,
) -> None:
    """
...
def format_color(
    color: Union[ColorInputType, Any], warn_if_invalid: bool = True
) -> Union[ColorType, Any]:
    """
...
def get_finger_pos(
    menu: Optional["pygame_menu.Menu"], event: EventType
) -> Tuple2IntType:
    """
...
def widget_terminal_title(
    ...
    if isinstance(widget, pygame_menu.widgets.Frame):
        ...
        w_title += f"{0} - {3}[{1},{2},".format(
            w_class_id, *widget.get_indices(), TerminalColors.LGREEN
        )
